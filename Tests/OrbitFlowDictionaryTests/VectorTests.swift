...
/// Runs the behavioural contract in `dictionary-test-vectors.json`.
///
/// The vectors are the specification for correction behaviour — fixed cases with
/// expected output, so a change in the corrector that alters results has to change this
/// file too, deliberately, rather than silently.
struct VectorTests {
